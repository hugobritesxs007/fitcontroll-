...
function renderRelatorios() {
    ...
    var vencidos    = appData.lotes.filter(function(l) { return diasParaVencer(l.dataValidade) <= 0; });
    var perdasReais = vencidos.reduce(function(acc, l) { 
        return acc + (parseFloat(l.qtdAtual) * parseFloat(l.custo)); 
    }, 0);
    var cupsUsados  = appData.cupons.filter(function(c) { return c.status === 'Utilizado'; }).length;
    var perdasEvit  = 4200.00;
    
    setText('rel-vendas',  moeda(totalVendas));
    setText('rel-perdas',  moeda(perdasReais));
    setText('rel-evitado', moeda(perdasEvit));
    setText('rel-cupons',  cupsUsados);
    ...
    appData.clientes.forEach(function(c) {
        var vc  = appData.vendas.filter(function(v) { return v.idCliente == c.id; });
        var tot = vc.reduce(function(acc, v) { return acc + (parseFloat(v.total) || 0); }, 0);
        var cups = vc.filter(function(v) { return v.idCupom !== null; }).length;
        ...
        rows += '<tr>' +
            ...
            '</div></td>' +
            '<td>' + vc.length + '</td>' +
            '<td class="font-700">' + moeda(tot) + '</td>' +
            '<td>' + (cups || '<span class="text-muted">—</span>') + '</td>' +
        '</tr>';
    });
    
    tbody.innerHTML = rows || '<tr><td colspan="4" style="padding:20px;text-align:center;color:var(--gray-400)">Sem dados</td></tr>';
}
